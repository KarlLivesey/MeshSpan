...
it("creates a swarm and keeps one-time recovery material visible until acknowledged", async () => {
  ...
  mountPage({ createMeshSetup, joinMeshSetup: vi.fn() }, complete, vi.fn());
...
function labelledInput(label: string, selector: string): HTMLInputElement {
  const labelled = [...document.querySelectorAll("label")].find((candidate) =>
    candidate.textContent.includes(label),
  );
  const control = labelled?.querySelector(selector);
