...
use meshspan_api_contract::{
    ApiError, ApiErrorCode, AuthenticationMethodId, CreateTotpRegistrationChallengeRequest,
...
use crate::{
    AuthenticationRegistrationStoreError, BrowserAuthenticationError, TotpRegistrationController,
    TotpRegistrationError, totp_registration_api_router,
};

const CHALLENGE_OPERATION: &str = "00000000-0000-4000-8000-000000000191";
const REGISTRATION_OPERATION: &str = "00000000-0000-4000-8000-000000000192";
...
const REGISTRATION_ROUTE: &str = "/api/latest/users/current/authentication-methods/totp";

#[tokio::test]
async fn boundaries_return_only_validated_secret_and_commit_results()
-> Result<(), Box<dyn std::error::Error>> {
    let calls = Arc::new(AtomicUsize::new(0));
    let router = totp_registration_api_router(FakeController::new(
        FakeOutcome::Success,
        Arc::clone(&calls),
    ))?;
    ...
    let challenge_body =
        serde_json::from_slice::<CreateTotpRegistrationChallengeResponse>(&challenge_bytes)?;
    ...
    assert_eq!(challenge_body.secret, "JBSWY3DPEHPK3PXPJBSWY3DPEHPK3PXP");
    ...
    let registration_body =
        serde_json::from_slice::<CreateTotpRegistrationResponse>(&registration_bytes)?;
    assert_eq!(
...
#[tokio::test]
async fn boundaries_reject_untyped_and_excessive_input_before_work()
-> Result<(), Box<dyn std::error::Error>> {
    let calls = Arc::new(AtomicUsize::new(0));
    let router = totp_registration_api_router(FakeController::new(
        FakeOutcome::Success,
    ...
    let oversized = router
        .oneshot(json_request(REGISTRATION_ROUTE, vec![b' '; 513])?)
        .await?;
    assert_error(
        oversized,
        StatusCode::BAD_REQUEST,
...
#[tokio::test]
async fn boundaries_map_closed_failures_without_secret_detail()
-> Result<(), Box<dyn std::error::Error>> {
    let cases = [
        (
    ...
    for (outcome, status, code) in cases {
        let response = totp_registration_api_router(FakeController::new(
            outcome,
...
impl FakeController {
    ...
    fn result<T>(&self, value: T) -> Result<T, TotpRegistrationError> {
        match self.outcome {
            FakeOutcome::Success => Ok(value),
            FakeOutcome::Rejected => Err(TotpRegistrationError::Authentication(
                BrowserAuthenticationError::Rejected,
            )),
            FakeOutcome::Conflict => Err(TotpRegistrationError::Conflict),
            FakeOutcome::Unavailable => Err(TotpRegistrationError::Unavailable),
            FakeOutcome::Failed => Err(TotpRegistrationError::Store(
...
impl TotpRegistrationController for FakeController {
    fn create_registration_challenge(
        ...
        self.result(CreateTotpRegistrationChallengeResponse {
            operation_id: request.operation_id.clone(),
            challenge_id: TotpRegistrationChallengeId::from_uuid_bytes(versioned(147))
                .ok_or(TotpRegistrationError::InvalidReceipt)?,
            secret: "JBSWY3DPEHPK3PXPJBSWY3DPEHPK3PXP".to_owned(),
            provisioning_uri: concat!(
                "otpauth://totp/MeshSpan%3Aowner?",
            ...
            digits: 6,
            period_seconds: 30,
            expires_at_epoch_micros: 1_700_000_120_000_000,
        })
    }
    ...
    fn register_totp(
        ...
        self.calls.fetch_add(1, Ordering::SeqCst);
        require_browser_headers(headers)?;
        self.result(CreateTotpRegistrationResponse {
            operation_id: request.operation_id.clone(),
...
fn require_browser_headers(headers: &HeaderMap) -> Result<(), TotpRegistrationError> {
