...
pub(crate) fn issue_recovery_codes_with<A>(
    authority: &mut A,
    issuance_key: &RecoveryCodeIssuanceKey,
    gateway: GatewaySessionIdentity,
    request: &CreateRecoveryCodesRequest,
    ...
    A: RecoveryCodeIssuanceAuthority,
{
    ...
    let codes = derive_codes(issuance_key, capability.principal_id, operation_id)?;
    let method_id = method_id(capability.principal_id, operation_id)?;
    let existing = authority.resolve_recovery_code_issuance(operation_id)?;
    ...
    let commit = match existing {
        Some(commit) => commit,
        None => authority.commit_or_resolve_recovery_code_issuance(context, &command)?,
    };
    validate_commit(
        commit,
