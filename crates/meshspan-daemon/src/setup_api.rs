...
use meshspan_api_contract::{
    ApiError, ApiErrorCode, ApiErrorIssue, BoundaryError, CreateMeshSetupRequest,
    CreateMeshSetupResponse, MAX_CREATE_MESH_SETUP_BYTES, OperationId as ApiOperationId,
    SetupState, SetupStatusResponse, decode_create_mesh_setup_request, encode_api_error,
    encode_create_mesh_setup_response, encode_setup_status_response, generate_openapi,
};
...
fn json_response(status: StatusCode, body: Vec<u8>, schema_digest: HeaderValue) -> Response<Body> {
...
fn boundary_issues(error: BoundaryError) -> Vec<ApiErrorIssue> {
    match error {
...
fn issue(path: &str, constraint: &str) -> ApiErrorIssue {
    ApiErrorIssue {
        path: path.to_owned(),
