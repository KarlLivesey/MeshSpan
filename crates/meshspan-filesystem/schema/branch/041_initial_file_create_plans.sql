...
-- A new volume has no namespace commit until its first visible mutation. File creation needs the
-- same durable initial-head representation as directory creation.
CREATE TABLE adapter_file_create_plans_v41 (
    ...
    expected_existing_object_id BLOB NULL CHECK (
        expected_existing_object_id IS NULL OR length(expected_existing_object_id) = 16
    )
) STRICT;

CREATE TABLE adapter_file_create_plan_ancestors_v41 (
    operation_id BLOB NOT NULL REFERENCES adapter_file_create_plans_v41(operation_id),
...
INSERT INTO adapter_file_create_plans_v41 SELECT * FROM adapter_file_create_plans;
INSERT INTO adapter_file_create_plan_ancestors_v41 SELECT * FROM adapter_file_create_plan_ancestors;

DROP TABLE adapter_file_create_plan_ancestors;
DROP TABLE adapter_file_create_plans;

ALTER TABLE adapter_file_create_plans_v41 RENAME TO adapter_file_create_plans;
ALTER TABLE adapter_file_create_plan_ancestors_v41 RENAME TO adapter_file_create_plan_ancestors;
